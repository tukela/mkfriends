...
function initExobud(){
 ...
 showTLab();
 disp1.innerHTML = "ExoBUD MP(II) v4.1tc+ 青城交友网1.0bate3";
 if(blnStatusBar){ window.status=('ExoBUD MP(II) v4.1tc+ 青城交友网1.0bate3');}
...
function wmpStop(){
 ...
 Exobud.close();
 disp1.innerHTML = "ExoBUD MP(II) v4.1tc+ 青城交友网1.0bate3";
 if(blnStatusBar){ window.status=('ExoBUD MP(II) v4.1tc+ 青城交友网1.0bate3');return true;}
}
...
function SetVolume(index)
{
	...
	if(ps.volume == 0 && !ps.mute){ps.mute = true;disp2.innerHTML="音量恢复"; imgChange("vmute", 1);}
	tidMsg=setTimeout('rtnTLab()',1000);
}
...
function wmpMute(){  
	...
	if(wmps == 3){clearInterval(tidTLab);}
	var ps = Exobud.settings;
	if(!ps.mute){ps.mute = true;disp2.innerHTML="开启静音模式"; imgChange("vmute", 1);}
	else {ps.mute = false;disp2.innerHTML="关闭静音模式"; imgChange("vmute", 0);}
	tidMsg=setTimeout('rtnTLab()',1000);
}
//function wmpMute(){
// var wmps = Exobud.playState;
...
function openPlist(){
 window.open("list.htm","mplist","top=10,left=280,width=350,height=500,scrollbars=no,resizable=yes,copyhistory=no");
}
