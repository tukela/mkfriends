...
while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	...
	{*/
if ( $_SGLOBAL['supe_sex']=='1'){
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('space')." where uid<>'$_SGLOBAL[supe_uid]' ORDER BY lastlogin DESC LIMIT 0,42");
}
else
{
	$query = $_SGLOBAL['db']->query("SELECT A.uid,A.username ,A.lastlogin,B.sex FROM ".tname('space')." AS A,".tname('spacefield')." AS B where  A.uid = B.uid and sex !=2  ORDER BY A.lastlogin DESC LIMIT 0,42");
}
while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	realname_set($value['uid'], $value['username']);
	$onlinelist[] = $value;
